...
void draw() {
  ...
  filteredImg1 = filter.getBilateralImage(src);
  filteredImg2 = filter.getBilateralImage(src);

  //2 - re-use bilateral filter on x Loop
  int loop = 25;
  for (int i=0; i<loop; i++) {
    filteredImg1 = filter.getBilateralImage(filteredImg1);
    filteredImg2 = filter.getBilateralImage(filteredImg2);
  }

  //3- Use Hue segmentation
  filteredImg1 = filter.getHueSegmentationImage(filteredImg1);
  filteredImg2 = filter.getHueSegmentationImage(filteredImg2, value);
