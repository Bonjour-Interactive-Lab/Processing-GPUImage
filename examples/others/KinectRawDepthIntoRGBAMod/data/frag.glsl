...
void main(){
	...
	vec4 tex = texture2D(dataTexture, uv);
	...
	#define iteration 2
	float smoothing = 0.25 / float(iteration);
	float thickness = 0.5 / float(iteration);
	...
	vec3 RGB = mix(vec3(0.1, 0.0, 0.0), vec3(0.0, 1.0, 1.0), DepthSlice.r) * distanceTex;
