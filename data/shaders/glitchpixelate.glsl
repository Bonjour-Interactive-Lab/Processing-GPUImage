...
/*PIXELATE*/
vec3 pixelate(vec2 uv, vec3 color, float index, float edge, float pixelRes){
	float offset = step(edge, index);
    ...
    float u = floor(uv.x / dx) * dx;
    float v = floor(uv.y / dy) * dy;

    uv = vec2(u, v);
    vec3 pixelateColor = texture(texture, uv).rgb;
    return mix(color, pixelateColor, vec3(offset)).rgb;
}
...
void main(){
	...
	vec3 pixel 		= pixelate(uv, tex.rgb, randIndex, INVERTINTENSITY, pixelres);
