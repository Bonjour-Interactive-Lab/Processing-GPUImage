...
/*DISPLACE RGB*/
vec3 displace(vec2 uv, vec2 deviation, float index, float edge){
	float offset = step(edge, index);
	vec2 doubleSideDeviation = mix(-deviation, deviation, vec2(index)) * 2.0 - 1.0;
	vec2 nuv = fract(uv + doubleSideDeviation * offset);
...
void main(){
	...
	vec3 cdisplace	= displace(waveuv, deviation, randIndex, INVERTINTENSITY);
