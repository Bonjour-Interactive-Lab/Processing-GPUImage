...
uniform vec2 deviation  = vec2(0.01);
...
/*DISPLACE LUMA*/
vec3 displaceLuma(vec2 uv, vec2 deviation, float index, float edge, float greyEdge, float grey){
	float offset = step(edge, index);
	vec2 doubleSideDeviation = mix(-deviation, deviation, vec2(index)) * 2.0 - 1.0;
	
	float stepGrey = step(greyEdge, grey);

	vec2 nuv = fract(uv + (doubleSideDeviation * stepGrey) * offset);

	return texture(texture, nuv).rgb;
}
...
void main(){
	...
	float grey 		= getLuma(tex.rgb);
	vec3 cdisplace 	= displaceLuma(waveuv, deviation, randIndex, INVERTINTENSITY, INVERTINTENSITY, grey);

	fragColor = vec4(cdisplace, 1.0);
}