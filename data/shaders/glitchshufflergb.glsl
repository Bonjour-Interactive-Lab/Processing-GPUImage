...
/*SHUFFLE RGB*/
vec3 shuffleRGB(vec3 color, float index, float edge, float contrast){
	float 	offset = step(edge, index);
	vec3 shuffle = clamp(color.brg + (index * contrast- dot(color, vec3(index))) * index, vec3(0.0), vec3(1.0));
	...
	gbr
	 */
	return shuffle * offset + color * (1 - offset);
}
...
void main(){
	...
	vec3 cshuffle	= shuffleRGB(tex.rgb, randIndex, INVERTINTENSITY, contrast);

	fragColor = vec4(cshuffle, 1.0);
}