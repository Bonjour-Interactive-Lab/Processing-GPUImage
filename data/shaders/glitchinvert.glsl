...
/*INVERT*/
vec3 invert(vec3 color, float index, float edge){
	float offset = step(edge, index);
	vec3 invert = vec3(1.0, 1.0, 1.0) - color;

	return invert * offset + color * (1 - offset);
}
...
void main(){
	...
	vec3 cinvert	= invert(tex.rgb, randIndex, INVERTINTENSITY);

	fragColor = vec4(cinvert, 1.0);
}